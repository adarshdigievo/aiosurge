...
import aiosurge
from aiosurge.api_resource import APIResource, PROJECTS_ENDPOINT
...
from aiosurge.errors import SurgeMissingIDError, SurgeMissingAttributeError
...
@pytest.mark.asyncio
async def test_update_with_arbitrary_parameters():
    ...
    with patch.object(Project, "put") as mock_put:
        ...
        await project.update(params={"allow_purgatory_users": True})
...
@pytest.mark.asyncio
async def test_update_with_fields_template():
    ...
    with patch.object(Project, "put") as mock_put:
        ...
        updated = await project.update(fields_template="ABC")
        mock_put.assert_called_once_with(
            f"{PROJECTS_ENDPOINT}/{project.id}",
            {"fields_text": "ABC"},
            api_key=None,
