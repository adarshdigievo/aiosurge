...
from aiosurge.projects import Task
...
def test_init_basic():
    t = Task(
        id="XYZ-123-ABC", project_id="ABC1234", created_at="2021-01-22T19:49:03.185Z"
    )
    ...
    assert t.created_at == datetime(2021, 1, 22, 19, 49, 3, 185000, tzinfo=tzutc())


def test_init_complete():
    response_json = {
        "id": "eaa44510-c8f6-4480-b746-28a6c8defd4c",
        "project_id": "A1B2C3-abcd-1234-wxyz-5823gd2238ac",
        "status": "completed",
        "created_at": "2021-01-22T19:49:42.000Z",
        "is_gold_standard": False,
        "fields": {
            "id": "163",
            "video": "https://streamable.com/m/tyler-glasnow-foul-to-austin-barnes-xzJURv",
        },
        "gold_standards_data": None,
        "is_complete": True,
        "responses": [
            {
                "id": "6db8b28d-bddc-491f-81ba-dbeb9e9f4399",
                "data": {"What is the result of this pitch?": "Foul Tip"},
    ...
    assert t.created_at == datetime(2021, 1, 22, 19, 49, 42, 000, tzinfo=tzutc())
...
def test_print_attrs():
    ...
    assert (
        attr
        == 'id="XYZ-123-ABC" project_id="ABC1234" created_at="2021-01-22 19:49:03.185000+00:00"'
    )
...
def test_repr():
    ...
    assert (
        attr
        == '<surge.Task#XYZ-123-ABC project_id="ABC1234" created_at="2021-01-22 19:49:03.185000+00:00">'
    )
